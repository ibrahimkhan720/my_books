                  ...
                  <div class="form-group">
                    ...
                    <select class="form-control" name="author_id" id="author_id" style="width: 100%;">
                       @foreach ($authores as $authore)
                          <option value="{{ $authore->id }}"{{ ($book->author_id == $authore->id) ? 'selected' : null }}>{{ $authore->title }}</option>     
                      @endforeach
                    </select>
                  </div>
                    <div class="form-group @error('status') has-error @enderror">
                        ...
                        <select class="form-control select2" name="status" id="status" style="width: 100%;">
                            <option value="0" {{ ($book->status == '0') ? 'selected' : null}}> none </option>     
                          <option value="3" {{ ($book->status == '3') ? 'selected' : null}}> upcoming </option>          
                        </select>
                     </div>
                  ...
                  <input type="text" class="form-control" name="publisher" id="publisher" placeholder="Publisher" value="{{ $book->publisher }}">
                      ...
                      <div class="form-group">
                        ...
                        <select class="form-control select2" name="country_of_publisher" id="country_of_publisher" style="width: 100%;">
                            <option value="none" {{ ($book->country_of_publisher == 'none') ? 'selected' : '' }}>-- Select Country --</option>
                            @foreach ($countories as $countory)
                                
                            <option value="{{ $countory->name }}" {{ ($book->country_of_publisher == $countory->name) ? 'selected' : '' }}>{{ $countory->name }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="form-group">
                  ...
                  <div class="form-group">
                    ...
                    <select class="form-control" name="recommended" id="recommended" style="width: 100%;">
                        <option value="none" {{ ($book->recommended == 'none') ? 'selected' : '' }}>-- Select Recommended --</option>
                        <option value="yes" {{ ($book->recommended == 'yes') ? 'selected' : '' }}>Recommended</option>
                        <option value="no" {{ ($book->recommended == 'no') ? 'selected' : '' }}>Not Recommended</option>
                    </select>
              </div>
